...
var reqTimer = setInterval(function wakeUp() {
  httpplease.get("https://oviahealth-apps-retrobot.herokuapp.com", function(err, res) {
    console.log("WAKE UP DYNO");
  });
  return reqTimer = setInterval(wakeup, 1800000);
}, 1800000);
...
function captureRetrospective(message) {
  ...
  if (message.edited || message.subtype === 'message_deleted') {
    ...
    delete retrospective.star[message.ts];
...
if (cmd === anonymous) {
  if (isAnonymous){
    ...
    if(inRetro){
      isAnonymous = true;
      return reply(message, 'The remaining itmems for this retro will be taken and reported anonymously.');
    }
    isAnonymous = true;
          ...
          slack.users.getPresence({ token, user }, (error, presence) => {
            ...
            slack.im.open({ token, user }, (error, data) => {
              ...
              reply({ channel: dms[user] }, '👂 Tell me about your retrospective' +
                '\nStart lines with `+` for "Start", `-` for "Stop", and `*` for "Continue".');
...
function flushRetrospective(retrospective) {
  ...
  const star = shuffleArray(Object.keys(retrospective.star).map(_ => retrospective.star[_]));

  debug('flushing retrospective (plus: %s, minus: %s, star: %s)', plus.length, minus.length, star.length);

  const source = { channel: channel || bot.self.channel };

  const messages = [];

  messages.push(`*Below is the "Start" list, :rocket:*`);
  plus.map((res, i) => {
    messages.push(`${i + 1}. ${res.line}`);
  });

  messages.push(`*Below is the "Stop" list, :facepalm:*`);
  minus.map((res, i) => {
    messages.push(`${i + 1}. ${res.line}`);
  });

  messages.push(`*Below is the "Continue" list, good work team 👍*`);
  star.map((res, i) => {
    messages.push(`${i + 1}. ${res.line}`);
  });
